...
class TestDefaultRequiredTrueSchema < SimpleJsonSchemaBuilder::Base
  object default_required: true do
    ...
    object :subtest do
      ...
      string :test5, required: false
    end

    string :test6, required: false
  end
end
  ...
  describe "required by default" do
    describe "when true" do
      ...
      it "correctly formats schema" do
        ...
        expect(properties[:subtest][:properties][:test5][:type]).to eq("string")

        expect(properties[:test6][:type]).to eq("string")
      end
    end
